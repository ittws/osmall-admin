<template>
    <div>
        ...
        <Button type="success" v-if="row.status=='禁用'" size="small" @click="enable(row)">启用</Button>
        <Button type="warning" v-if="row.status=='启用'" size="small" @click="disable(row)">禁用</Button> 
    ...
    export default {
        ...
        methods: {
            edit (row) {
                this.$router.push({
                    name: 'shippingedit',
                    params: {shippingName:row.shippingName}
                });
            },
            del () {
                this.delModel=true;
